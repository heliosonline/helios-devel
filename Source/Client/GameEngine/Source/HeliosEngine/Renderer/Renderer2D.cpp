...
namespace Helios {
	...
	void Renderer2D::DrawChar(const glm::mat4& transform, const glm::vec4* vertices, const Ref<Texture2D>& texture, const glm::vec2* texCoords, const glm::vec4& color)
	{
		...
		// seems somewhere wrong ... ???
		float tx_size_x = (texCoords[1].x - texCoords[0].x) / (1.0f / texture->GetWidth()) * 10.0f;
		float tx_size_y = (texCoords[2].y - texCoords[0].y) / (1.0f / texture->GetHeight()) * 10.0f;
		...
		for (size_t i = 0; i < textVertexCount; i++)
		{
			...
			s_Data.TextVertexBufferPtr->TexSize = glm::vec2(tx_size_x, tx_size_y);// texSize[i];
